/**
 * Alak's LAYOUT, split out from its text on purpose.
 *
 * `SurahConfig.ts` derives a long list of module-level constants from this
 * object (`PAGE_HEIGHT`, `layoutMath`, the styling figures), and the whole
 * canvas imports those — so this file is unavoidably part of the bundle every
 * surah page loads.
 *
 * Alak's VERSE TEXT is not, and that is the point of the split. While the two
 * lived in one module, that shared bundle carried ~650 lines of Arabic, Turkish
 * and English verse data for a surah the reader may never open, because a
 * module is pulled in whole and this half of it was needed. The text now lives
 * in `alak96Config.ts`, which is reached only through `surahDatabase`'s dynamic
 * import, exactly like every other surah.
 *
 * Keep it that way: import layout from here, text from there, and never let one
 * file need both.
 */
...
export const ALAK_LAYOUT_CONFIG: SurahLayoutConfig = {
  ...
  // (ALAK_SIDE_INFO_EN) lives in `alak96Config.ts` alongside the verse text and
  // is attached to this object at the bottom of that file. It is left empty
  // here on purpose: this module is loaded by every page, that one only when
  // Alak is actually opened, which is the whole reason the two are separate.
