...
export function SurahCarousel({ surahs }: { surahs: SurahCardData[] }) {
  ...
  return (
    <div className="w-full max-w-[1200px] relative flex items-center justify-center group">
      {/* Left Arrow */}
      <button 
        onClick={scrollLeft}
      ...
      <div 
        ref={scrollRef}
        className="w-full flex gap-4 md:gap-8 overflow-x-auto snap-x snap-mandatory py-6 px-14 md:px-24 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
      >
        {surahs.map((surah, index) => (
          <div key={surah.id} className="min-w-[260px] md:min-w-[340px] w-[75vw] max-w-[340px] snap-center flex-shrink-0 transition-transform duration-300 hover:scale-[1.02]">
